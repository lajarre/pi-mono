...
- Fixed Kitty inline images inside tmux by wrapping Kitty graphics sequences in tmux passthrough when `$TMUX` is set, reusing stable Kitty image IDs across rerenders, and restoring the terminal cursor to the reserved image row after rendering so Ghostty/Kitty/WezTerm images no longer duplicate or leave following content vertically offset when tmux `allow-passthrough` is enabled
