...
- Fixed Kitty inline images inside tmux by wrapping Kitty graphics sequences in tmux passthrough when `$TMUX` is set and reusing stable Kitty image IDs across rerenders, restoring Ghostty/Kitty/WezTerm image rendering when tmux `allow-passthrough` is enabled
