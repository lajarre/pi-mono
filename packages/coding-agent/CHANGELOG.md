...
- Fixed Kitty inline images inside tmux by wrapping Kitty graphics sequences in tmux passthrough when `$TMUX` is set, reusing stable Kitty image IDs across rerenders, and documenting the tmux `allow-passthrough` requirement for Ghostty/Kitty/WezTerm terminals
