...
- Fixed Kitty inline images inside tmux by wrapping Kitty graphics sequences in tmux passthrough when `$TMUX` is set, reusing stable Kitty image IDs across rerenders, restoring cursor position after image draws to avoid vertically offset follow-up content, and documenting the tmux `allow-passthrough` requirement for Ghostty/Kitty/WezTerm terminals
